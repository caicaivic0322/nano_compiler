...
void CodeEditor::updateHighlighter()
{
    ...
    // 根据文件类型设置高亮规则
    if (m_fileExtension == "c" || m_fileExtension == "h") {
        m_highlighter->setLanguage(SyntaxHighlighter::CLanguage);
    } else if (m_fileExtension == "cpp" || m_fileExtension == "hpp" || m_fileExtension == "cc") {
        m_highlighter->setLanguage(SyntaxHighlighter::CppLanguage);
    }
}
