...
#include "ui/repl_panel.h"
#include "interpreter/mp_wrapper.h"
...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_replPanel(nullptr)
    , m_interpreter(std::make_unique<MpWrapper>())
    , m_isModified(false)
    , m_isRunning(false)
    , m_statusLabel(nullptr)
    , m_fileTypeLabel(nullptr)
{
    setupUI();
    setupConnections();
    ...
    // 显示欢迎信息
    m_outputPanel->appendMessage("欢迎使用 nanoPython!", OutputPanel::Info);
    m_outputPanel->appendMessage("点击 '运行' 按钮执行 Python 代码", OutputPanel::Info);
...
void MainWindow::setupStatusBar()
{
    ...
    m_fileTypeLabel = new QLabel("Python", this);
...
{
    // 连接解释器信号
    connect(m_interpreter.get(), &MpWrapper::executionStarted, this, [this]() {
        m_runAction->setEnabled(false);
        m_stopAction->setEnabled(true);
        m_statusLabel->setText("正在运行...");
    });
    
    connect(m_interpreter.get(), &MpWrapper::outputReceived, this, [this](const QString &output) {
        m_outputPanel->appendMessage(output, OutputPanel::Normal);
    });
    
    connect(m_interpreter.get(), &MpWrapper::executionFinished, this, &MainWindow::onExecutionFinished);
}
...
void MainWindow::onNewFile()
{
    ...
    if (editor) {
        // 插入默认的 Python 框架代码
        QString defaultCode = R"(#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
void MainWindow::onRun()
{
    ...
    m_outputPanel->clear();
    m_interpreter->execute(filePath, sourceCode);
}

void MainWindow::onStop()
{
    m_interpreter->stop();
    m_statusLabel->setText("运行已停止");
}

void MainWindow::onAbout()
{
    QMessageBox::about(this, "关于 nanoPython",
        "<h2>nanoPython 1.0.0</h2>"
        "<p>轻量级 Python IDE</p>"
        "<p>专为低分辨率显示器和性能一般的电脑优化</p>"
        "<p>支持多标签页（最多3个）</p>"
        "<p>内置 MicroPython 解释器，开箱即用</p>");
}
...
void MainWindow::onExecutionFinished(bool success, const QString &output)
{
    m_runAction->setEnabled(true);
    m_stopAction->setEnabled(false);
    
    if (success) {
        m_statusLabel->setText("运行成功");
    } else {
        m_statusLabel->setText("运行失败");
    }
}
